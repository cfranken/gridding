...
function main()
	...
	for d in startDate:dDay:stopDate
	    ...
	    for a in files
			...
			biome = fin["biomes"].var[:]'
			println(maximum(biome[:,8]))
			println(size(biome))
			dim = size(lat_in_)
			...
			# Biome sum:
			b_all = sum(biome[:,8:10],dims=2)
			# Get indices within the lat/lon boudning box:
			idx = findall((minLat[:,1].>latMin).&(maxLat[:,1].<latMax).&(minLon[:,1].>lonMin).&(maxLon[:,1].<lonMax).&(b_all[:,1].>75))
