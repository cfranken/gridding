...
function main()
	...
	println("Output file dimension (lon/lat):")
    println(length(lon)," ", length(lat))
    # Create output file:
    dsOut = Dataset("testGrid.nc","c")
	...
	for (key, value) in dGrid
		println(key)
		NCDict[key] = defVar(dsOut,key,Float32,("lon","lat"),deflatelevel=4, fillvalue=-999)
	end

	#dSIF = defVar(dsOut,"sif",Float32,("lon","lat"),deflatelevel=4, fillvalue=-999)
	dN = defVar(dsOut,"n",Float32,("lon","lat"),deflatelevel=4, fillvalue=-999)
    # Define data array
    ...
    files = glob("TROPO_SIF_2018-0[7]-??_*.nc", ar["folder"])
	...
	dN[:,:]=mat_data[:,:,end]./nGrid^2
	co = 1
	for (key, value) in dGrid
		NCDict[key][:,:]=round.(mat_data[:,:,co]./mat_data[:,:,end],sigdigits=5)
		NCDict[key][dN.<1].=-999
		co += 1
	end
    close(dsOut)
